...
example_mcmc_log_file <- "../eden/logs/death_rate_validation2_pop_10000_mu_1_dr_0.205_n_50_state_clock_estimate_dr.log"

example_mcmc_log <- as.data.frame(readLog(example_mcmc_log_file))

# Get true simulated rates
#From extract_validation_sims_rates.R
true_example_diff_df <- read_tsv("../eden/stats/validation_growth_and_death_rates_weighted_large.tsv") %>% 
...
example_birth_rates_diff_posteriors_summary <- example_mcmc_log %>% 
    ...
    dplyr::summarise(mean = mean(birthRateDiff),
                  ...
                  hpd_90_upper = hdi(birthRateDiff,
                                     credMass =0.9)[2])
# Example edge - center difference posterior distribution
...
ggsave(plot=example_birth_rates_diff_posteriors, file ="../figures/example_birth_rates_diff_posteriors_large.png", height = 5, width = 4.5)

ggsave(plot=example_birth_rates_posteriors, file ="../figures/example_birth_rates_posteriors_large.png", height = 5, width = 4.5)
...
mcc_tree <- read.beast("../eden/trees/death_rate_validation_pop_10000_mu_1_dr_0.205_n_50_state_clock_estimate_dr.typed.node.mcc.tree")
...
ggsave(plot=g_pie, file ="../figures/example_ancestral_state_recon_tree_large.png", height = 5, width = 5)
...
example_alive_cells <- read_csv("../eden/simulation_data/alive_cells_death_rate_validation_pop_10000_mu_1_dr_0.195.csv")

### Ancestral state reconstruction ######

sim_tree <- readRDS("../eden/simtrees/cells_death_rate_validation_pop_10000_mu_1_dr_0.195.rds")
...
example_tumor_inset
ggsave(plot=example_tumor_inset, file ="../figures/example_tumor_edge_center_sampled_large.png", height = 5, width = 5)
...
ggsave(plot=true_versus_post_prob, file ="../figures/example_true_versus_post_prob_ancestor_recon_large.png", height = 5, width = 5)
...
#sim_rates <- read_tsv("../eden/stats/posteriors2/posterior_summary.tsv")

#From extract_validation_sims_rates.R
weighted_sim_rates <- read_tsv("../eden/stats/validation_growth_and_death_rates_weighted_large.tsv") %>% 
    dplyr::mutate(true_birth_rate_diff_weighted = mean_edge_birth_rate - mean_center_birth_rate) %>% 
    dplyr::select(dr, true_birth_rate_diff_weighted)
...
clock_comparison_growth_rate_posteriors_df <- read_tsv(file = "../eden/stats/posteriors2/posterior_summary.tsv")%>% 
...
clock_rate_comparison_posteriors_plot <- clock_comparison_growth_rate_posteriors_df %>% 
    dplyr::filter(n==50, minBirthRateESS > 250, clock_model == "state_dependent") %>% 
...
ggsave(file = "../figures/clock_rate_comparison_birth_rate_posteriors_large.png",
...
ggsave(file = "../figures/clock_comparison_sample_size_mse_large.png",
