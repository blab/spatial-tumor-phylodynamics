##visualize_eden_extended_power_analysis.R
...
clock_rate_comparison_posteriors_plot <- clock_comparison_growth_rate_posteriors_df %>% 
    ...
    ggplot(., aes(x=true_birth_rate_diff, y=mean_birth_rate_diff, color = clock_model)) +
...
ggsave(file = "/Users/mayalewinsohn/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/figures/clock_rate_comparison_birth_rate_posteriors.png",
...
clock_comparison_sample_size_results_summary <- clock_comparison_growth_rate_posteriors_df %>% 
    ...
    summarise("mse" = mean((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE),
              "se_mse" = sd((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE) / sqrt(n()))
...
ggsave(file = "/Users/mayalewinsohn/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/figures/clock_comparison_sample_size_mse.png",
       clock_comparison_sample_size_mse, height = 5, width = 5)
#visualize true versus estimated rates
birth_rate_sampling_fig <- growth_rate_posteriors %>%            
    filter(minBirthRateESS > 100, n <= 50) %>% 
    ...
    theme(text = element_text(size = 15)) +
    facet_wrap(~n)
birth_rate_sampling_fig
ggsave(file = "/Users/mayalewinsohn/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/figures/birth_rate_sample_size.png", birth_rate_sampling_fig, height = 5, width = 7)


sample_size_results_summary <- growth_rate_posteriorsdf %>% 
...
birth_rate_sampling_fig <- strict_clock_growth_rate_posteriors %>%            
...
sample_size_results_summary <- growth_rate_posteriors_df %>% 
    filter(minBirthRateESS > 200) %>% 
    group_by(n) %>% 
    summarise("mse" = mean((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE),
              "se_mse" = sd((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE) / sqrt(n()))
...
strict_clock_sample_size_results_summary <- strict_clock_growth_rate_posteriors_df %>% 
    filter(minBirthRateESS > 100) %>% 
    group_by(n) %>% 
    summarise("mse" = mean((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE),
              "se_mse" = sd((mean_birth_rate_diff - true_birth_rate_diff)^2, na.rm = TRUE) / sqrt(n()))
...
sample_size_mse  <- sample_size_results_summary %>% 
    filter(as.numeric(n) <= 90) %>% 
    ggplot(., aes(x = n, y = mse)) + geom_point() +
    theme_classic() + xlab("Sample size") + ylab("MSE of edge - center birth rate difference") +
    geom_errorbar(aes(ymin = mse - se_mse, ymax = mse + se_mse)) +
    theme(text = element_text(size = 15)) +
    scale_x_continuous(breaks = pretty(sample_size_results_summary$n, n = 10))
sample_size_mse

strict_clock_sample_size_mse  <- strict_clock_sample_size_results_summary %>% 
    filter(as.numeric(n) <= 100) %>% 
    ggplot(., aes(x = n, y = mse)) + geom_point() +
    theme_classic() + xlab("Sample size") + ylab("MSE of edge - center birth rate difference") +
    geom_errorbar(aes(ymin = mse - se_mse, ymax = mse + se_mse)) +
    theme(text = element_text(size = 15))+
    scale_x_continuous(breaks = pretty(strict_clock_sample_size_results_summary$n, n = 10))
strict_clock_sample_size_mse

ggsave("~/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/figures/sample_size_power_analysis_extended.png", sample_size_mse, height = 5, width = 5)

sample_size_mse_zoom  <- sample_size_results_summary %>% 
    filter(as.numeric(n) <= 20) %>% 
    ggplot(., aes(x = n, y = mse)) + geom_point() +
    theme_classic() + xlab("Sample size") + ylab("") +
    geom_errorbar(aes(ymin = mse - se_mse, ymax = mse + se_mse)) +

    theme(text = element_text(size = 15)) +
    scale_x_continuous(breaks = pretty(sample_size_results_summary$n, n = 4))
sample_size_mse_zoom
ggsave("~/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/figures/sample_size_power_analysis_extended_zoom.png", sample_size_mse_zoom, height = 5, width = 5)
