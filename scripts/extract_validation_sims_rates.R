...
extract_growth_rates_dr_validation <- function(all_cells_file, time_interval = 2/24, cutoff_frac = 0.1) {
    ...
    return(data.frame("mean_edge_growth_rate" = mean_edge_growth_rate,
...
growth_rates <- purrr::map(sim_cells_files, function(all_cells_file) extract_growth_rates_dr_validation(all_cells_file, cutoff_frac = 0.25))
...
write.csv(growth_rates_df,
          file = "outputs/beast_analysis/state_dependent_clock_model/validation/validation_growth_and_death_rates_weighted.csv")
