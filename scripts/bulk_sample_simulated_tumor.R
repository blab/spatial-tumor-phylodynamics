...
get_punch_sequence <- function(punch_id_indices, sampled_cells, ref_seq, ccf = 0.15) {
  ...
  punch_sequence <- paste(map_chr(1:nchar(sampled_cells$collapsed_sequence[1]), function(pos) get_position_bp(pos = pos, punch_cells = punch_cells, ref_seq = ref_seq, ccf = ccf)), collapse = "")
  return(punch_sequence)
}
bulk_sample_and_sequence <- function(all_cells, punch_radius_n_cells = 2, n_samples = 20, min_alt_reads = 2, min_coverage = 10, depth = 100, ccf = 0.15, seed = NA) {
    ...
    sampled_cells$collapsed_sequence <- map_chr(sampled_cells$sequence, extract_sequences)
