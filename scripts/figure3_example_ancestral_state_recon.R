...
## Tree for schematic, not saved directly
# a <- ggtree(tree_boundary_driven) + geom_point(aes(color = ifelse(death_edge_state == 1, "edge", "center")), size = 3) +
#     scale_color_manual(values = colors_edge_center) +
#     theme(legend.position = "none")
#
# a <- viewClade(a, MRCA(a, "cell_2779", "cell_1564"))
# a
...
#labs(color = "P(cell death)") 
d_baseplot
ggsave(plot=d_baseplot, file ="manuscript/figures/physicell_2D_neutral_bdg_base.png", height = 5, width = 6)
d_lowdeath <- ggdraw() +
...
ggsave(plot=d_full, file ="manuscript/figures/physicell_2D_neutral_bdg.png", height = 5, width = 6)
...
e_baseplot <- growth_rate_posteriors_3D_net_bdg_n100 %>% 
    ...
    geom_abline(intercept = 0, slope = 1, linetype = "dashed") + xlab("True bifth rate difference (edge - center)") +
    theme(text=element_text(size=15)) +
    ylab("Estimated birth rate difference (edge - center)") +# ggtitle("Physicell 3D Neutral Boundary-Driven Growth") +
    ylim(-0.001, 0.004) ##TODO: take this away when we figure out what's going with sims
e_baseplot
ggsave(plot=e_baseplot, file ="manuscript/figures/physicell_3D_neutral_bdg_base.png", height = 5, width = 6)
#labs(color = "P(cell death)") 
e_baseplot
e_lowdeath <- ggdraw() +
...
f_baseplot <- growth_rate_posteriors_2D_sel_bdg_n100 %>% 
    ...
    geom_point(aes( y = mean_birth_rate_diff), alpha = 0.8, size = 2) + scale_color_viridis(option = "mako", discrete = TRUE, direction = -1) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed") + xlab("True birth rate difference (edge - center)") +
    ylab("Estimated birth rate difference (edge - center)") + theme(text=element_text(size =15)) + ylim(0,0.0065) #ggtitle("Physicell 2D Selection Boundary-Driven Growth") +

f_baseplot
ggsave(plot=f_baseplot, file ="manuscript/figures/physicell_2D_sel_bdg_base.png", height = 5, width = 6)

growth_rate_posteriors_2D_sel_bdg_n100 %>% 
    filter(mean_birth_rate_diff < 0, minBirthRateESS > 150) %>% 
    select(dr,i, mean_birth_rate_diff, birthRate_hdi95_lower, birthRate_hdi95_upper)
...
####### SAMPLE SIZE POWER ANAYLSIS PLOT #####

sample_size_analysis_files <- paste0("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/logs/", 
                                     list.files(path = "/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/logs",
                                                pattern = "estimate_dr.log"), sep = "")
sample_size_analysis_files2 <- paste0("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/logs2/", 
                                     list.files(path = "/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/logs2",
                                                pattern = "estimate_dr.log"), sep = "")
library(beastio)
library(HDInterval)
library(coda)
sample_size_mcmc_logs  <- purrr::map(sample_size_analysis_files, beastio::readLog)
sample_size_mcmc_logs2  <- purrr::map(sample_size_analysis_files2, beastio::readLog)


calc_mean_estimated_birth_rate <- function(mcmc_log, log_file) {
    ...
    estimatedBirthRateDiffPosteriors <- mcmc_log[,"birthRateCanonical.1"] - mcmc_log[,"birthRateCanonical.0"]
    return(data.frame(mean_estimated_birth_rate_diff = mean(estimatedBirthRateDiffPosteriors),
                      HPD95_estimated_birth_rate_lower = hdi(estimatedBirthRateDiffPosteriors)[1],
                      HPD95_estimated_birth_rate_upper = hdi(estimatedBirthRateDiffPosteriors)[2],
                      ess = min(effectiveSize(mcmc_log)[c("birthRateCanonical.0", "birthRateCanonical.1")]),
           dr = dr_extract, 
           n = n_extract))
}

sample_size_results_df <- purrr::map2(sample_size_mcmc_logs, sample_size_analysis_files,
                                      function(mcmc_log, log_file) calc_mean_estimated_birth_rate(mcmc_log = mcmc_log, log_file = log_file) )

sample_size_results_df2 <- purrr::map2(sample_size_mcmc_logs2, sample_size_analysis_files2,
                                      function(mcmc_log, log_file) calc_mean_estimated_birth_rate(mcmc_log = mcmc_log, log_file = log_file) )

sample_size_results_df2 <- sample_size_results_df2 %>% 
    bind_rows %>% 
    mutate(dr = as.numeric(dr)) %>% 
    left_join(.,sim_rates, by = "dr")

sample_size_results_df <- sample_size_results_df %>% 
    bind_rows %>% 
    mutate(dr = as.numeric(dr)) %>% 
    left_join(.,sim_rates, by = "dr")

sample_size_results_df_comb <- bind_rows(list(sample_size_results_df, sample_size_results_df2)) %>% 
    arrange(-ess) %>% 
    distinct


sample_size_results_df_comb  <- sample_size_results_df_comb  %>% 
    mutate(mean_edge_birth_rate = mean_edge_growth_rate + dr,
           mean_center_birth_rate = mean_center_growth_rate + dr) %>% 
    mutate(mean_birth_rate_diff = mean_edge_birth_rate - mean_center_birth_rate)

sample_size_results_df_comb %>% 
    filter(ess > 100) %>% 
    ggplot(., aes(x = mean_birth_rate_diff, y = mean_estimated_birth_rate, color = n)) +
    geom_point() +
    facet_wrap(facets = "n", nrow = 2) +
    geom_errorbar(aes(ymin = HPD95_estimated_birth_rate_lower, ymax = HPD95_estimated_birth_rate_lower.1)) +
    theme_classic() +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed")
library(yardstick)

sample_size_results_summary <- sample_size_results_df_comb %>% 
    filter(ess > 200) %>% 
    group_by(n) %>% 
    summarise("mse" = mean((mean_birth_rate_diff - mean_estimated_birth_rate)^2, na.rm = TRUE),
              "se_mse" = sd((mean_birth_rate_diff - mean_estimated_birth_rate)^2, na.rm = TRUE) / sqrt(n()))
              # "r_squared" = rsq(.,mean_estimated_birth_rate, mean_birth_rate_diff)$.estimate,
              # "sample_size" = rsq(.,mean_estimated_birth_rate, mean_birth_rate_diff)$n) #%>% 
    # ungroup %>% 
    # dplyr::select(-n) %>% 
    # distinct()


sample_size_mse  <- sample_size_results_summary %>% 
    filter(as.numeric(n) < 100) %>% 
    ggplot(., aes(x = n, y = mse)) + geom_point() +
    theme_classic() + xlab("Sample size") + ylab("MSE") +
    geom_errorbar(aes(ymin = mse - se_mse, ymax = mse + se_mse))

# sample_size_r_squared <- ggplot(sample_size_results_summary, aes(x = sample_size, y = r_squared)) + geom_point() +
#     theme_classic() + xlab("Sample size") + ylab("R^2")

sample_size_mse 



ggsave(plot=sample_size_mse, file ="manuscript/figures/sample_size_mse.png", height = 5, width = 4.5)


################ EXAMPLE POSTERIOR ##################
#example_mcmc_log <- as.data.frame(readLog(sample_size_analysis_files[196]))
#example_mcmc_log_file <- sample_size_analysis_files[196]
...
true_eden_rates <- read_csv("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/validation_growth_and_death_rates_weighted.csv")
true_example_birth_rate_diff <- true_eden_rates[true_eden_rates$dr == "0.29",] %>% 
    mutate(mean_birth_rate_diff= mean_edge_growth_rate - mean_center_growth_rate) %>% 
    select(mean_birth_rate_diff)
...
example_birth_rates_diff_posteriors <- example_mcmc_log %>% 
    ...
    geom_vline(xintercept = true_example_birth_rate_diff[[1]], linetype = "dashed")
...
birthRateRatiologs <- example_mcmc_log %>% 
    dplyr::mutate(birthRateRatio = birthRateCanonical.1 /  birthRateCanonical.0)

#50% HPD interval
coda::HPDinterval(as.mcmc(birthRateRatiologs$birthRateRatio), prob = .50)
#90% HPD interval
coda::HPDinterval(as.mcmc(birthRateRatiologs$birthRateRatio), prob = .90)
#true 
true_eden_rates <- read_csv("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/validation_growth_and_death_rates_weighted.csv")
true_example_birth_rate_ratio <- true_eden_rates[true_eden_rates$dr == "0.29",] %>% 
    mutate(mean_birth_rate_ratio = mean_edge_growth_rate / mean_center_growth_rate) %>% 
    select(mean_birth_rate_ratio)

print(true_example_birth_rate_ratio )
...
#f1_score <- F1_Score(node_conversion_df$is.edge,node_conversion_df$true_state)
...
true_versus_post_prob <- ggplot(node_conversion_df, aes(edge_prob, true_state, color = ifelse(true_state == 1, "edge", "center"))) +
    ...
    #          label = paste0("F1score == ", format(f1_score , digits = 2)),
...
library(tidyverse)
dr = "0.00"
get_anc_recon_comparison <- function(dr) {
    print(dr)
    sim_tree <- readRDS(paste0("~/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/analysis/simtrees/cells_death_rate_validation_pop_1000_dr_", dr, ".rds"))
    beast_tree <- read.beast(paste0("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/beast_analysis/state_dependent_clock_model/validation/logs2/death_rate_validation_pop_1000_dr_",
                                    dr,"_n_100_state_clock_estimate_dr.typed.node.trees.mcc"))
    
    # all_cells <- read_csv(paste0("/Volumes/BALAENA/projects/spatial_tumor_growth_simulation/outputs/raw_simulation_results/validation/cells_death_rate_validation_pop_1000_dr_",
    #
    sampled_sim_tree <- prune_simulated_tree(sim_tree, sampled_cells = as.numeric(gsub("loc[01]", "", gsub("cell","",  beast_tree@phylo$tip.label))))
    #gsub("loc[01]", "", gsub("cell","",  mcc_tree@phylo$tip.label))
    #example_alive_cells$sampled <- example_alive_cells$index %in% as.numeric(gsub("loc[01]", "", gsub("cell","",  mcc_tree@phylo$tip.label)))
    ### Ancestral state reconstruction ######
    
    node_conversion_df <- match_beast_to_sim_tree(sim_tree = sampled_sim_tree, beast_tree = beast_tree) %>% 
        add_column("dr" = as.numeric(dr))
    # logistic_model <- glm(true_state ~ edge_prob, node_conversion_df, family = binomial)
    # Confusion_DF <- ConfusionDF(node_conversion_df$is.edge, node_conversion_df$true_state)
    # Precision <- Precision(node_conversion_df$is.edge, node_conversion_df$true_state)
    # f1_score <- MLmetrics::F1_Score(node_conversion_df$is.edge, node_conversion_df$true_state)

    return(node_conversion_df)
    
}
dr_vec <- format(seq(0.1, 0.6, by = 0.01), digits = 2)
anc_recon_comparison_df <- map(dr_vec, get_anc_recon_comparison) %>% 
    bind_rows

example_drs <- c(0.1, 0.2, 0.3, 0.4)
anc_recon_comparison_df_subset <- anc_recon_comparison_df %>% 
    dplyr::filter(dr %in% example_drs)
logistic_model <- glm(true_state ~ edge_prob, anc_recon_comparison_df_subset, family = binomial)
model_fit <- data.frame("edge_prob" = seq(0, 1, by = 0.01))
model_fit$true_state <- predict(logistic_model, newdata=model_fit, type="response")
library(viridis)
anc_recon_comparison_plot <- anc_recon_comparison_df %>% 
    dplyr::filter(dr %in% example_drs) %>% 
    #ggplot(., aes(edge_prob, true_state, color = ifelse(true_state == 1, "edge", "center"))) +
    ggplot(., aes(edge_prob, true_state, color = dr)) +
    geom_jitter(width = 0, height = 0.1, alpha = 0.8) + theme_classic() + #scale_color_manual(values = colors_edge_center) +
     xlab("Posterior probability edge") + ylab("True state") + xlim(0, 1)+
    geom_line(data=model_fit,color = "black") +
    theme(text = element_text(size = 20)) +
    scale_color_viridis(
        option = "rocket", end = 0.8)
ggsave(plot=anc_recon_comparison_plot, file ="manuscript/figures/example_true_versus_post_prob_ancestor_recon.png", height = 5, width = 6)   
# dr_vec <- format(seq(0, 0.8, by = 0.01), digits = 2)
# fscore_vec <- c()
# for (dr in dr_vec) {
#     f1 <- calc_f1_score_anc(dr)
#     
#     fscore_vec <- c(fscore_vec, f1)
# }
# f1_scores <- map(dr_vec, calc_f1_score_anc ) %>% 
#     bind_rows
# sim_trees_files <- list.files(path="manuscript/analysis/simtrees/",
#                               pattern = "[0-9].rds", include.dirs = TRUE, full.names = TRUE)
# sim_trees_files <- sim_trees_files[(! grepl("unrestricted", sim_trees_files)) & (! grepl("_i_", sim_trees_files))]
# library(MLmetrics)
# r2_tjur(logistic_model)
# 
# library(caret)
...
# full_sim_tree <- readRDS("~/Documents/PhD/Bedford_lab/spatial_tumor_growth_simulation/manuscript/analysis/simtrees/cells_death_rate_validation_pop_1000_dr_0.29.rds")
# sampled_sim_tree <- prune_simulated_tree(full_sim_tree, sampled_cells = example_sampled_cells$index)
# daughter_cell <- "cell_4531"
# get_cells_on_branch <- function(daughter_cell, sampled_sim_tree, full_sim_tree) {
#     full_sim_daughter_node <- full_sim_tree@data$node[full_sim_tree@data$cell_name==daughter_cell]
#     full_sim_daughter_ancestor_nodes <-  phangorn::Ancestors(full_sim_tree@phylo, node = full_sim_daughter_node, type = "all")
#     full_sim_daughter_ancestor_cell_names <- full_sim_tree@data$cell_name[match(full_sim_daughter_ancestor_nodes, full_sim_tree@data$node)]
#     sampled_sim_daughter_node <- which(sampled_sim_tree@data$cell_name == daughter_cell)
#     sampled_sim_parent_node <- phangorn::Ancestors(sampled_sim_tree@phylo, node = full_sim_daughter_node, type = "parent")
#     sampled_sim_parent_cell_name <- sampled_sim_tree@data$cell_name[sampled_sim_parent_node]
#     
#     
#     
# }
